...
public class ChessFrame extends JFrame{
	...
	public ChessFrame(){
		...
		for (int i = 0; i < 8; i++){
			...
			for (int j = 0; j < 8; j++){ 
				if (i == 1) chessBoard[i][j] = new ChessButton(new Square(j, i), DEFAULT_BUTTON_SIZE, current, new ImageIcon("img/bp.png"));
				else if (i == 6) chessBoard[i][j] = new ChessButton(new Square(j, i), DEFAULT_BUTTON_SIZE, current, new ImageIcon("img/wp.png"));
			    else chessBoard[i][j] = new ChessButton(new Square(j, i), DEFAULT_BUTTON_SIZE, current);
				current = (current == color1) ? color2 : color1;
